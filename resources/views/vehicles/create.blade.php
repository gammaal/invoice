...
<div class="bg-white rounded-xl shadow-sm border border-gray-100">
    <div class="px-6 py-4 border-b border-gray-100">
        <h2 class="text-sm font-semibold text-gray-800">Form Tambah Kendaraan</h2>
    </div>
    <form action="{{ route('vehicles.store') }}" method="POST" class="p-6 space-y-4">
        @csrf
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number" name="year" value="{{ old('year') }}"
                    placeholder="{{ date('Y') }}" min="1980" max="{{ date('Y')+1 }}"
        ...
        <div>
            ...
            <textarea name="notes" rows="2"
                class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
                placeholder="Catatan kondisi atau spesifikasi khusus">{{ old('notes') }}</textarea>
        </div>
