    ...
    <div class="lg:col-span-2 space-y-5">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-xs font-bold text-gray-500 uppercase tracking-wider">Informasi Kendaraan</h3>
                <span class="font-mono text-xs text-blue-600 font-semibold">{{ $invoice->invoice_number }}</span>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="mechanic_id"
                        ...
                        @foreach($mechanics as $m)
                            <option value="{{ $m->id }}" {{ old('mechanic_id',$invoice->mechanic_id)==$m->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="number" name="mileage_in" value="{{ old('mileage_in',$invoice->mileage_in) }}"
                        min="0" placeholder="Contoh: 45000"
                        class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">KM Keluar</label>
                    <input type="number" name="mileage_out" value="{{ old('mileage_out',$invoice->mileage_out) }}"
                        min="0" placeholder="Isi setelah selesai"
                        class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <textarea name="complaint" rows="2"
                        class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none">{{ old('complaint',$invoice->complaint) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Diagnosa Mekanik</label>
                    <textarea name="diagnosis" rows="2"
                        class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none">{{ old('diagnosis',$invoice->diagnosis) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Kondisi Kendaraan</label>
                    <textarea name="vehicle_condition" rows="2"
                        class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none">{{ old('vehicle_condition',$invoice->vehicle_condition) }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="space-y-3">
                ...
                <div>
                    ...
                    <input type="number" name="discount" id="discount"
                        value="{{ old('discount',$invoice->discount) }}" min="0" step="500"
                ...
                <div>
                    ...
                    <input type="number" name="tax" id="tax"
                        value="{{ old('tax',$invoice->tax) }}" min="0" step="500"
                        class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
...
const CURRENT_VEHICLE_ID = {{ $invoice->vehicle_id ?? 'null' }};

function loadVehicles(customerId, selectVehicleId) {
    ...
    fetch(`/vehicles/by-customer/${customerId}`)
        ...
        .then(list => {
            ...
            list.forEach(v => {
                ...
                if (v.id == (selectVehicleId || CURRENT_VEHICLE_ID)) o.selected = true;
                sel.appendChild(o);
            });
        });
}
...
function buildOptions(selectedId) {
    ...
    if (services.length) { html += '<optgroup label="🔧 Jasa">'; services.forEach(p => { html += `<option value="${p.id}" data-price="${p.price}" data-unit="${p.unit}" ${p.id==selectedId?'selected':''}>${p.name}</option>`; }); html += '</optgroup>'; }
    if (parts.length)    { html += '<optgroup label="🔩 Spare Part">'; parts.forEach(p => { html += `<option value="${p.id}" data-price="${p.price}" data-unit="${p.unit}" ${p.id==selectedId?'selected':''}>${p.name}</option>`; }); html += '</optgroup>'; }
    return html;
}

function createRow(index, productId='', qty=1, price=0) {
    return `<tr class="product-row border-t border-gray-100" data-index="${index}">
        <td class="px-3 py-2"><select name="products[${index}][product_id]" class="w-full px-2 py-1.5 text-xs border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 product-select" onchange="onProductChange(this)">${buildOptions(productId)}</select></td>
        <td class="px-3 py-2"><div class="flex items-center gap-1"><input type="number" name="products[${index}][quantity]" value="${qty}" min="0.1" step="0.5" class="w-16 px-2 py-1.5 text-xs border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 qty-input" oninput="recalcRow(this.closest('tr'))"><span class="text-xs text-gray-400 unit-label whitespace-nowrap"></span></div></td>
        <td class="px-3 py-2"><input type="number" name="products[${index}][price]" value="${price}" min="0" step="500" class="w-28 px-2 py-1.5 text-xs border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 price-input" oninput="recalcRow(this.closest('tr'))"></td>
        <td class="px-3 py-2 text-xs font-semibold text-gray-900 subtotal-display whitespace-nowrap">Rp 0</td>
        <td class="px-3 py-2"><button type="button" onclick="removeRow(this)" class="text-red-400 hover:text-red-600"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button></td>
    </tr>`;
}
...
function addRow(productId='', qty=1, price=0) {
    ...
    rowIndex++;
    if (productId) { const p = PRODUCTS.find(x => x.id==productId); if (p) row.querySelector('.unit-label').textContent = p.unit; }
    recalcRow(row);
}
function removeRow(btn) { if (document.querySelectorAll('.product-row').length<=1){alert('Minimal 1 item!');return;} btn.closest('tr').remove(); recalcAll(); }
function onProductChange(select) {
    const opt = select.options[select.selectedIndex]; const row = select.closest('tr');
...
function recalcAll() {
    let sub=0;
    document.querySelectorAll('.product-row').forEach(r => { sub += (parseFloat(r.querySelector('.qty-input').value)||0)*(parseFloat(r.querySelector('.price-input').value)||0); });
...
// Load existing data
loadVehicles({{ $invoice->customer_id }}, {{ $invoice->vehicle_id ?? 'null' }});
...
@else
    @foreach($invoice->details as $d)
        addRow({{ $d->product_id }},{{ $d->quantity }},{{ $d->price }});
    @endforeach
@endif
</script>
@endsection
